...
작성 주차는 `WEEKLY_TEST_DATE`를 기준으로 다가오는 일요일에 지정됩니다.
일요일에는 당일이 지정됩니다. 따라서 승인 직후 같은 주차를 기준으로
...
2. `weekly-test:down`을 실행하지 않은 상태에서 날짜를 다음 날인
   `2026-08-03`으로 변경합니다.
3. `npm run weekly-test:up`을 다시 실행해 Bot과 Backend를 가상 월요일로
   재기동합니다.
...
삭제와 추가 작업 수정은 해당 주차가 끝나는 월요일 00:00 전까지만 허용됩니다.
...
해당 주차의 주간보고도 함께 삭제됩니다. 마감이 지난 일요일의 보고서가 없으면
